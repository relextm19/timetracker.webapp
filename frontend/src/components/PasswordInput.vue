<template>
    <div class="w-full min-h-[2.5rem] relative">
        <input
            ...
            :type="showPassword ? 'text' : 'password'" :placeholder="props.placeholder" v-model="model"
            @focus="focusHandler" ref="input">
        <button class="absolute right-3 top-1/2 transform -translate-y-1/2 text-white w-5"
            @click="showPassword = !showPassword" type="button">
            <img src="../assets/eye-crossed.png" alt="eye" v-if="!showPassword"
                :class="hasError ? 'eye-error' : 'filter invert'">
            <img src="../assets/eye.png" alt="eye" :class="hasError ? 'eye-error' : 'filter invert'" v-else>
        </button>
    </div>
</template>
<script setup lang="ts">
// TODO: change the eye icon to an svg
import { ref } from 'vue';
